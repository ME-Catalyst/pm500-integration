...
- `src/infrastructure/influxdb/.env` – set `DOCKER_INFLUXDB_INIT_USERNAME`, `DOCKER_INFLUXDB_INIT_PASSWORD`, and `DOCKER_INFLUXDB_INIT_ORG`.
- `src/node-red/.env` – define `BROKER_URL`, `INFLUXDB_URL`, and secret tokens for downstream systems.
- (Optional) `src/cloud/aws-iot/terraform.tfvars` – populate AWS region, IoT thing group, and certificate S3 bucket.

## 4. Provision Edge Services
```bash
cd src/infrastructure/influxdb
docker compose up -d
cd ../..
cd src/node-red
docker compose up -d
...
2. Use the *Import* dialog to load the flows under `src/node-red/flows/`, such as `assembly-to-influx.json` and `assembly-to-mqtt.json`.
...
2. Review `src/cloud/aws-iot/README.md` for prerequisites.
3. Apply Terraform modules:
   ```bash
   cd src/cloud/aws-iot/terraform
   terraform init
   terraform apply
   ```
