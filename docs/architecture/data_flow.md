...
This guide explains how the PowerMonitor 500 edge stack forwards measurements from field assets into analytics platforms using Node-RED, Telegraf, and InfluxDB.

![End-to-end telemetry data flow from field sources through the edge pipeline and MQTT distribution.](../visuals/diagrams/telemetry_data_flow.svg)
...
- **Local analytics:** Node-RED and Telegraf processors enable edge-level KPIs, alarms, and anomaly detection without cloud dependencies, keeping production insights available during WAN outages.
