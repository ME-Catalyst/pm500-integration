...
This document explains how the PowerMonitor 500 integration concept is constructed across field, edge, platform, and enterprise layers. It highlights the major components, their responsibilities, and the seams available for customization.

![Component overview of the PM500 integration stack showing field, edge, and cloud tiers.](docs/visuals/diagrams/component_overview.svg)
...
For channel-level responsibilities and credential ownership, review the [IO map diagram](docs/architecture/diagrams/io_map.svg) alongside the [module inventory](docs/architecture/diagrams/module_graph.md).
...
2. **Edge data services**
   ...
   - Telemetry bifurcates into MQTT Sparkplug B payloads for real-time consumers and InfluxDB line protocol for historian persistence.
...
| Cloud | AWS IoT Core | Terraform modules with optional CloudFormation wrappers | Multi-AZ managed service; certificate rotation automated through IoT Jobs. |

## Data Contracts
- **MQTT Sparkplug B:** Topic namespace `spBv1.0/pm500/<site>/<area>/<asset>`. Payload expectations are documented in [`docs/developer/internal_apis.md`](docs/developer/internal_apis.md).
...
- Principle of least privilege guides IAM roles; device certificates are restricted to telemetry publish permissions.
